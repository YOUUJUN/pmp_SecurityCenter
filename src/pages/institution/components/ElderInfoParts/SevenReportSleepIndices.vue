<template>
	<div class="sevenDays">
		<div v-if="showSeven" class="content">
				...
				<div class="nowListWrap">
					<div>
						<div class="nowList nowList_Ques">
							...
							<div class="deExplain">
								<div
									class="reportTrendsDataVo"
									id="reportTrendsDataVoSleep"
									style="height: 180px"
								></div>
							</div>
						</div>
							...
							<div class="nowTop alignItemStart">
								...
								<div class="titName ml10">
									<div>
										<div>
											最多离床次数
											<span class="colorFF7186">{{ outBedNum.count }}次</span>
										</div>
									</div>
								</div>
							</div>
							...
							<div class="nowTop alignItemStart">
								...
								<div class="titName ml10">
									<div>
										<div>
											平均入睡时长
											<span class="colorFF7186">{{ sleepStDuration.count }}</span>
											分钟
										</div>
									</div>
								</div>
...
export default {
	...
	updated() {
		...
		// this.setInExRatio(this.inExRatioDataInfo.xaxis, this.inExRatioDataInfo.data);
		// this.setSleepScore();
		// this.setAHI();
		// this.setFallIndex();
	},
	methods: {
		...
		initEcharts() {},
		setOption() {
			let chartDomSleep = document.getElementById('reportTrendsDataVoSleep')
			let myChartSleep = echarts.init(chartDomSleep)
			this.option = {
				tooltip: {
					trigger: 'axis',
					axisPointer: {
			...
			myChartSleep.setOption(this.option)
		...
		setWakeUpDuration() {
			let chartDomWakeUpDuration = document.getElementById('wakeUpDuration')
			let { xaxis, data, indexVal } = this.wakeUpDuration

			let myChartWakeUpDuration = echarts.init(chartDomWakeUpDuration)
			var option = {
				tooltip: {
					trigger: 'axis',
					axisPointer: {
			...
			myChartWakeUpDuration.setOption(option)
			window.addEventListener('resize', () => {
				myChartWakeUpDuration.resize()
			})
		},
				...
				.then(
					(res) => {
						if (res.status === 200 && res.data.result === 'success') {
							...
							const { inExRatioData } = inExRatioDataoBJ
							this.inExRatioDataInfo = inExRatioData
							// debugger;
							// --- 吸呼比 end---
							// 睡眠异常指数
							...
							this.errorMsgSeven = res.data.message
